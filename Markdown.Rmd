...
**NB** if the user changes the array defining `id_name_s`, then this section should be changed accordingly.  
...
### Graphic device
We decided to use the png device, but others such as pdf or tiff do the trick as well.
1. The line we would use produces a filename that includes the `file.out` substrig defined above as well as `GROUP` and the date.
2. For this document, we parametrised the string that generates the png file.
